...
| Local S3-compatible | MinIO (`quay.io/minio/minio`, pinned release) in docker compose with a bucket-init job; the same image via testcontainers for integration tests; `DOCULENS_TEST_S3_ENDPOINT_URL` reuses a running one                                                     |
