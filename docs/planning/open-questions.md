...
metadata filter.
**Retrieval phase note (2026-09-21):** retrieval takes the scope per question (documents, a
collection, or all) and resolves it to `READY` document ids exactly as proposed (ADR-016); how a
conversation stores that scope is still open.
...
three ports is mandatory for CI, and provider selection is config-driven.
**Retrieval phase note (2026-09-21):** the `RerankerProvider` port, the fail-open `RerankingStage`
and a deterministic fake exist (ADR-016); reranking is off by default (`RERANKER_PROVIDER=none`)
until a hosted vendor is chosen. If Anthropic is chosen, consult the
...
**Decided provisionally (2026-09-21, ADR-016):** PostgreSQL full-text search (`simple`
configuration, GIN expression index) behind the repository port, fused with the vector ranking
by reciprocal-rank fusion; `RETRIEVAL_STRATEGY` selects semantic, keyword or hybrid (default).
A language-specific configuration remains open until the corpus language is known.
...
**Retrieval phase note (2026-09-21):** the threshold half exists as `RETRIEVAL_MIN_SCORE` and an
empty `RetrievalResult` (ADR-016); the prompt half waits for the LLM provider.
...
| OQ-1                                                                                                                                                                                                      | Adapter decided (ADR-002): the `VectorStore` port, the ChromaDB adapter with adapter-enforced tenant isolation, one collection per embedding model and chunk-derived vector ids, plus an in-memory store for unit tests. Where the ChromaDB server runs when deployed (ECS/EC2 versus Chroma Cloud) is still open.                                                                              | 2026-09-20 | ADR-002 |
| OQ-17                                                                                                                                                                                                     | Partially decided (ADR-004): the `EmbeddingProvider` port, a deterministic fake for local development and CI, and an OpenAI-compatible adapter (OpenAI, Azure OpenAI, Ollama, vLLM) selected by `EMBEDDING_PROVIDER`. The production vendor (Bedrock versus a hosted API) and the LLM and reranker providers remain open.                                                                       | 2026-09-20 | ADR-004 |
...
| OQ-13                                                                                                                                                                                                     | Decided (ADR-014): MinIO via Chainguard (`cgr.dev/chainguard/minio`, digest-pinned) is the local S3-compatible store, in docker compose and via testcontainers; a filesystem adapter serves Docker-free development and unit tests. Official MinIO Hub/Quay images are no longer anonymously pullable.                                                                                          | 2026-09-26 | ADR-014 |
...
| OQ-8                                                                                                                                                                                                      | Retrieval half decided (ADR-016): a question is scoped to selected documents, one collection, or all of the owner's documents, resolved to `READY` document ids and passed to the vector store as an exact id filter. Conversation-level persistence of the scope remains open.                                                                                                                 | 2026-09-21 | ADR-016 |
| OQ-29                                                                                                                                                                                                     | Partially decided (ADR-016): `RETRIEVAL_MIN_SCORE` drops weak hits before any answer is attempted and an empty result is the explicit insufficient-evidence signal; whether the prompt also decides is settled with the LLM provider.                                                                                                                                                           | 2026-09-21 | ADR-016 |
| OQ-30                                                                                                                                                                                                     | Provisional (ADR-016): context items carry a stable 1-based `index`, prepared for numbered references; the attribution mechanism itself is decided with question answering.                                                                                                                                                                                                                     | 2026-09-21 | ADR-016 |
| OQ-4                                                                                                                                                                                                      | Provisional (ADR-016): keyword retrieval is PostgreSQL full-text search over chunk text (`simple` configuration, GIN index, OR-ed terms ranked by `ts_rank_cd`) behind `DocumentContentRepository.search_chunks`; `HybridRetriever` fuses it with vector retrieval by reciprocal-rank fusion; `RETRIEVAL_STRATEGY` chooses semantic, keyword or hybrid. Language-specific stemming is deferred. | 2026-09-21 | ADR-016 |
| OQ-17                                                                                                                                                                                                     | Further decided (ADR-005): the `LLMProvider` port (`generate` over ordered chat messages, structured errors, usage), a deterministic fake for local development and CI, and an OpenAI-compatible chat-completions adapter selected by `LLM_PROVIDER`; retries, timeouts and limits are configuration. The production vendor for embeddings, generation and reranking remains open.              | 2026-09-22 | ADR-005 |
...
| OQ-13                                                                                                                                                                                                     | Further decided (ADR-006): Redis is the local job-queue adapter; `memory` for tests; SQS when deployed. MinIO remains the local object store (ADR-014).                                                                                                                                                                                                                                         | 2026-09-26 | ADR-006 |
| OQ-27                                                                                                                                                                                                     | Provisional (ADR-006 / ADR-011): commit the document row, then enqueue; enqueue failure is non-fatal and a reconciliation sweep re-enqueues stragglers. Transactional outbox remains the upgrade path.                                                                                                                                                                                          | 2026-09-26 | ADR-006 |
| OQ-3                                                                                                                                                                                                      | Provisional (ADR-020): direct multipart `POST /api/v1/documents` capped by `MAX_FILE_SIZE_MB`; Lambda deployments must set the cap within platform body limits. Presigned upload remains the upgrade for 50 MB on Lambda. Download/view path still open.                                                                                                                                        | 2026-09-26 | ADR-020 |
