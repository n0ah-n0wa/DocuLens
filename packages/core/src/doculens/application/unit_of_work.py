...
class DocumentRepository(Protocol):
    ...
    async def search_for_owner(
        ...
        """The owner's documents whose filename contains ``query``, ignoring case (§29).

        Metadata search only: the query is matched literally, so wildcards a user types carry no
        special meaning. Semantic search over content is a separate feature (`OQ-10`).
        """
        ...
